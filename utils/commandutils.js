module.exports = class CommandUtils {
    ...
    static getMapFromArgs(args) {
        ...
        if (mapKeyFound) {
            ...
            if (remainingArgs.length > 2 && remainingArgs[1].length > 0) {
    ...
    /**
     * Takes a list of arguments and looks for a matching key and returns the following argument if found. 
     * 
     * @param {*} args 
     * @param {*} key 
     */
    static getArgAfterKey(args, key) {
        for (var i = 0; i < args.length; i++) {
    ...
    static getModeFromArgs(args) {
        return CommandUtils.getArgAfterKey(args, "-mode");
    }
